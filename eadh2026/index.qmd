...
- batches of 50 entries sent to the model, one batch at a time 
- chosen model: gpt-oss:120b (run locally)
...
# we processed the dataset again 

# and we got satisfactory results

## but...

- the model gpt-oss:120b kept outperforming competition (we thought)
...
## results: Latin replaced with Polish

![](img/LAT_vs_POL_with_trendlines.png)



## conclusions

- LLMs in annotation pipelines: very efficient
- messy datasets can get a new life now
- (modern) local LLMs better than we all thought
- the _Bibliography_ by Estreichers: suitable for large-scale studies
- the history of Polish prints can be re-written now (kinda)
...
- pull a model of your choice, e.g.:
    ...
    - `gemma4:31b` -- astonishing performance
    - `qwen3.6:35b` -- just 4b active parameters (=fast)
    - `qwen3.8:27b` -- a new kid on the block, amazingly strong
    - . . . and dozens of other models.
- to be run in a chatbot mode (known from chatGPT etc.)
- or to process your dataset in batches.
